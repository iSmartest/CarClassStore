...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
{
    ...
    public void doPullRefreshing(final long delayMillis)
    {
        if (isPullToRefreshEnabled)
        {
            postDelayed(new Runnable()
            {
                ...
                public void run()
                {
                    int newScrollValue = -getHeaderHeight();
                ...
                if (isReadyForPull())
                {
                    ...
                    if (yDiff > touchSlop && yDiff > xDiff)
                    {
                                ...
                                && isReadyForPullDown())
                        {
                            ...
                            if ((mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) && dy <= 0.0001f
                                    && isReadyForPullUp())
                            {
                                lastMotionY = y;
                                isBeingDragged = true;
    ...
    private void init(Context context, AttributeSet attrs)
    {
        ...
        mode = a.getInteger(R.styleable.PullToRefresh_mode, MODE_PULL_DOWN_TO_REFRESH);// mode
                                                                                       // =
                                                                                       // 0x3;
        ...
        if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH)
        {
            ...
            String pullLabel = context.getString(R.string.pull_to_refresh_header_1);
            String refreshingLabel = context.getString(R.string.pull_to_refresh_loading);
            String releaseLabel = context.getString(R.string.pull_to_refresh_header_2);
        ...
        {
            // Loading View Strings
            String pullLabel = context.getString(R.string.pull_to_refresh_footer_1);
            String refreshingLabel = context.getString(R.string.pull_to_refresh_loading);
            String releaseLabel = context.getString(R.string.pull_to_refresh_footer_2);
    ...
    private boolean pullEvent()
    {
        ...
        if (newHeight != 0)
        {
            if (state == PULL_TO_REFRESH && headerHeight < Math.abs(newHeight))
            {
                ...
                if (state == RELEASE_TO_REFRESH && headerHeight >= Math.abs(newHeight))
                {
                    state = PULL_TO_REFRESH;
