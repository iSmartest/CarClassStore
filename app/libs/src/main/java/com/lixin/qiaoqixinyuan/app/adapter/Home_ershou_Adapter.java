...
public class Home_ershou_Adapter extends BaseAdapter {
    ...
    private int code;
    public void setHousinginnews(List<Ershou_Bean.Secondnewslist> secondnews) {
        this.secondnews = secondnews;
    }

    private Context context;
    private List<Ershou_Bean.Secondnewslist> secondnews;
   public Home_ershou_Adapter( Dialog dialog,int code){
       this.dialog=dialog;
       this.code=code;
   }
    @Override
    public int getCount() {
        return secondnews == null ? 0 : secondnews.size();
    }
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        ImageLoader.getInstance().displayImage(secondnews.get(i).secondinnewsimage,vh.iv_icon, ImageLoaderUtil.DIO());
        vh.tv_name.setText(secondnews.get(i).secondinnewsitem);
        vh.tv_instruction.setText(secondnews.get(i).secondinnewsdetail);
        vh.tv_time.setText(secondnews.get(i).secondinnewstime);
        if (code==1) {
            vh.iv_delete.setVisibility(View.VISIBLE);
            vh.iv_delete.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    secondnewsid = secondnews.get(i).secondnewsid;
                    position = i;
                    deleterefundorder("1", secondnewsid, MyApplication.getuId());
                }
            });
        }else {
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        try {
                            ...
                            if ("1".equals(result)){
                                ...
                                secondnews.remove(position);
                                notifyDataSetChanged();
                                ToastUtil.showToast(context,"删除二手信息成功");
                            }
